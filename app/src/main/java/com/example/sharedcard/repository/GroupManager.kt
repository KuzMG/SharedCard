...
class GroupManager @Inject constructor(
    ...
    fun deleteUser(idUser: UUID, idGroup: UUID) {
        groupUsersDao.deleteUser(idGroup, idUser)
    }

    fun makeUserAdmin(idUser: UUID, idGroup: UUID) {
        groupUsersDao.setStatusOnAdmin(idUser, idGroup)
    }

    fun addUserInGroup() {
        val groupUsers = GroupUsersEntity(
            UUID.fromString("3f1c6374-2398-4c57-8e41-37976cd6a88f"),
            UUID.fromString("9489013c-5a4a-48d9-8bb4-ac29e4e4666b"),
            false
        )
